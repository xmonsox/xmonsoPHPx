    ...
    <div class="container mt-5">
        ...
        if(!empty($_POST['btnmodificar'])){
            if(!empty($_POST['nombre']) and !empty($_POST['contra'])){
                ...
                include_once("conexion.php");
                $sql=$conexion->query("update usuario set nombre='$nombre', contraseña='$contra' where id=$id");
                if ($sql==1) {
                    ?>
                    ...
                    <div class="text-center mt-4">
                        <a href="http://localhost/Taller_php/login/consulta_usu.php" class="btn btn-warning border border-secondary-subtle btn-lg ">Volver Consultar Usuario</a>
                    </div>
                    <?php
                }else{
                    ?>
