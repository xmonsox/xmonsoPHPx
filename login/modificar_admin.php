...
<body>
    <div class="container mt-5">
        ...
        if(!empty($_POST['btnmodificar'])){
            if(!empty($_POST['nombre']) and !empty($_POST['contra'])){
                $nombre = $_POST['nombre'];
                $contra = $_POST['contra'];
                
                include_once("admin_cone.php");
                $sql=$conexion->query("update admin set nombre='$nombre', contraseña='$contra' where id_admin=$id");
                if ($sql==1) {
                    ?>
                    ...
                    <div class="text-center mt-4">
                        <a href="http://localhost/Taller_php/login/consulta_admin.php" class="btn btn-warning border border-secondary-subtle btn-lg ">Volver Consultar Admnin</a>
                    </div>
                    <?php
                }else{
                    ?>
